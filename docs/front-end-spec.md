...
我们的应用不包含传统的页面或屏幕。其全部用户界面只有一个核心部分：

```mermaid
graph TD
    A["CLI 执行 (CLI Execution)"] --> B["状态栏输出 (Status Line Output)"];
    B --> C["宠物状态显示 (Pet Status Display)"];
    C --> D["本地状态保存 (Local State Persistence)"];
````
...
### **流程 2: 宠物状态重置流程 (Flow 2: Pet State Reset Flow)**

**用户目标 (User Goal):**
当宠物死亡后，系统会在下次CLI执行时自动应用时间衰减，用户无需手动干预。

**流程步骤 (Flow Steps):**

1.  用户的宠物能量归零，状态栏显示 `(x_x)`（死亡状态）。
2.  用户继续编码工作，Claude Code 定期调用CLI脚本刷新状态栏。
3.  CLI脚本检测到死亡状态，保持显示死亡表情。
4.  用户如需重置，可删除 `~/.claude-pet/pet-state.json` 文件，下次执行将创建新宠物。
...
#### **关键要求 (Key Requirements)**
  ...
  * **交互 (Interaction):** 无直接交互界面，宠物状态通过Claude Code状态栏自然显示，依赖Claude Code本身的无障碍支持。
  * **内容 (Content / Screen Readers):** CLI输出应当对屏幕阅读器友好。格式建议为：`(^_^) 状态宠物: 开心, 能量: 85%`，Claude Code状态栏会适当处理显示。

#### **测试策略 (Testing Strategy)**

  * **手动测试:** 在Claude Code的多种主题配置下检查可读性。
  * **屏幕阅读器测试:** 依赖Claude Code自身的无障碍支持，CLI输出应简洁明确。

## **8. 响应式策略 (Responsiveness Strategy)**

响应式指**适应Claude Code终端窗口宽度变化导致的状态栏空间变化**。
...
#### **性能目标 (Performance Goals)**
  ...
  * **CLI执行时间:** \< 100毫秒。
  * **文件I/O响应:** \< 50毫秒。
  * **资源占用:** 无持久进程，每次执行后立即释放资源。
  * **状态栏刷新:** 不影响Claude Code主进程性能。
