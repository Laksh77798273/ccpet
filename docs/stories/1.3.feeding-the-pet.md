...
- `src/extension.ts` - Enhanced ClaudeCodeStatusLine with JSONL token processing [Source: architecture/source-tree.md#cli-entry-point]
...
- Enhanced CLI integration in `src/extension.ts` must have comprehensive tests
...
- CLI integration tests: `src/__tests__/extension.test.ts` (updated for stdin processing)
...
- src/extension.ts - Enhanced ClaudeCodeStatusLine with stdin processing and JSONL integration
...
- src/__tests__/extension.test.ts - Updated for stdin processing and JSONL integration tests
