...
- [ ] 1.1 在IPetState接口中添加petName字段定义
- [ ] 1.2 在core/config.ts中定义PET_NAMES常量数组（至少20个名称）
- [ ] 1.3 创建generateRandomPetName()实用函数
- [ ] 1.4 添加名称生成的单元测试覆盖
- [ ] 1.5 确保TypeScript类型检查通过所有现有代码

### Task 2: 更新Pet类支持宠物名称 (AC: 3)
- [ ] 2.1 修改Pet构造函数处理petName初始化逻辑
- [ ] 2.2 更新_createInitialState方法包含随机名称生成
- [ ] 2.3 在resetToInitialState方法中为新宠物分配新名称
- [ ] 2.4 更新所有相关的私有方法确保petName正确处理
- [ ] 2.5 验证现有Pet类功能无回归

### Task 3: 升级PetStorage类支持名称持久化 (AC: 4)
- [ ] 3.1 更新loadState方法处理petName字段反序列化
- [ ] 3.2 添加向后兼容逻辑：为缺少petName的状态生成随机名称
- [ ] 3.3 更新saveState方法确保petName被序列化
- [ ] 3.4 测试新旧状态文件的加载兼容性
- [ ] 3.5 验证状态文件格式正确性
...
- [ ] 5.1 编写Pet.test.ts中的petName相关测试用例
- [ ] 5.2 编写PetStorage.test.ts中的名称持久化测试
- [ ] 5.3 创建向后兼容性集成测试
- [ ] 5.4 验证名称生成的随机性和分布
- [ ] 5.5 执行端到端测试确保系统稳定
